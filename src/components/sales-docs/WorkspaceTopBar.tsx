...
import { AudioLines, ChevronDown, ClipboardList, FileText, Pause, Square } from "lucide-react";
...
export default function WorkspaceTopBar({
  ...
  return (
    <div className="flex h-[52px] shrink-0 items-center justify-between border-b border-[var(--sd-border)] bg-[var(--sd-bg)] px-4">
      {/* Tabs */}
      <div className="flex h-full items-end gap-1">
        ...
        <button className="flex items-center gap-2 px-3 pb-3.5 text-[13px] text-[var(--sd-text-muted)] transition-colors hover:text-[var(--sd-text-secondary)]">
      ...
      <div className="flex items-center gap-2.5">
        <div className="flex items-center gap-2 rounded-full border border-[var(--sd-border-strong)] bg-[var(--sd-panel-raised)] py-1.5 pl-3 pr-3.5">
          <AudioLines
            size={14}
        ...
        <button className="sd-ghost-btn flex h-8 w-8 items-center justify-center rounded-full">
          <Pause size={13} strokeWidth={2.2} />
        </button>
        <button className="flex h-8 w-8 items-center justify-center rounded-full bg-[var(--sd-red)] text-white transition-[filter] hover:brightness-110">
